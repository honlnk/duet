 ...
 * 运行前需先启动服务: PORT=3001 npm run dev
 */
import { createSession, saveSession, loadSession, listSessions, deleteSession } from '../server/src/store/sessionStore.js'
...
async function runSession(topic, agents, config, { stopAfterChunk = false, waitMs = 60000 } = {}) {
...
async function main() {
  ...
  check('chunk 流式事件收到', r1.events.some(e => e.type === 'chunk'), `${r1.events.filter(e=>e.type==='chunk').length} chunks`)

  // [验收2] 流式输出（仅 content，无 reasoning）
  console.log('\n【验收2】流式输出仅 content')
  const hasReasoningInChunk = r1.events.some(e => e.type === 'chunk' && e.content === null)
  check('chunk 均带 content 文本', !hasReasoningInChunk && r1.events.some(e => e.type === 'chunk'))
  check('消息内容不含思维链标记', r1.session.messages.every(m => m.content && !m.content.includes('reasoning')))

  // [验收3] 设置轮数能准确停
  console.log('\n【验收3】轮数上限准确停止')
  check('maxRounds=1 停在 1 round', r1.session.finishedReason === 'max_rounds', `reason=${r1.session.finishedReason}`)
  ...
  check('A 生成了独立摘要', !!r5.session.memory.A.summary, `${r5.session.memory.A.summary?.length || 0} 字`)
  check('B 生成了独立摘要', !!r5.session.memory.B.summary, `${r5.session.memory.B.summary?.length || 0} 字`)
  check('A 摘要是第一人称', /我/.test(r5.session.memory.A.summary || ''), '含"我"')
  check('A 摘要不含 B 的 persona 原文', !(r5.session.memory.A.summary || '').includes('你主张晚练'))
  ...
  check('停止后 reason=stopped', r7.session.finishedReason === 'stopped', `reason=${r7.session.finishedReason}`)
  ...
  check('totalTokens > 0', (statsEvents.at(-1)?.totalTokens || 0) > 0, `${statsEvents.at(-1)?.totalTokens}`)
  check('estCost 有值', (statsEvents.at(-1)?.estCost || 0) > 0, `$${statsEvents.at(-1)?.estCost}`)

  // [验收10] 部署
  console.log('\n【验收10】部署模式')
  const indexResp = await fetch(`${API}/`).then(r => r.text())
  check('首页返回 HTML', indexResp.includes('<!DOCTYPE html>') && indexResp.includes('Duet'))
  const healthResp = await fetch(`${API}/api/health`).then(r => r.json())
